...
function fetchItems(category){    
    ...
    return new Promise((resolve,reject)=>{   
        ...
        if(category === "all"){    
            ...
            items.where("categoryId","==",category).get().then((result)=>{            
                if(result.size === 0){            
                    reject("No hay resultados")
                }
                let res = result.docs.map((d) =>{ 
...
console.log("test entra");
    const testData = [    
       ...
       {     
        id: "4",   
        categoryId: "xiaomi",
        description: "Octa-core (4x2.0 GHz Kryo 260 Gold & 4x1.8 GHz Kryo 260 Silver)",     
       ...
       {        
        id: "1",
        categoryId: "xiaomi",
        description: "Glass front (Gorilla Glass 3), stainless steel frame, plastic back",     
